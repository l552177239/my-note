...
## 1. 引言
上一篇 [大模型](/pages/ai-llm/) 讲解基础API调用：Token、上下文、采样参数、提示词、RAG基础直觉。进入真实业务落地，会遇到几类典型诉求：
- 多步任务：先做分类，再抽取信息，最后生成回复
- 多轮对话：维护对话历史，同时控制Token开销，防止上下文溢出
- 扩展外部能力：查询数据库、调用HTTP接口、执行计算逻辑
- 私有知识库问答：检索业务文档，约束模型基于材料输出答案

**编排**就是将以上能力组装成完整可运行的应用流水线。
LangChain 是主流的LLM应用编排框架：提供统一抽象层对接各家大模型，把提示模板、链、记忆、工具、检索组件灵活组合。本篇目标建立概念模型，做到**可以搭建、可以定位问题**。

> 本篇与后续章节边界，避免内容冲突

| 本篇（阶段3） | 交由后续章节学习 |
|----------------|------------------|
| 顺序链、路由链、记忆、RAG流水线 | **LangGraph**：复杂分支、循环、人机审批流程 |
| **单Agent(ReAct)动态调用工具** | **多Agent、Skill业务案例**（阶段6） |
| LangChain内置工具封装 | **MCP协议**：标准化工具与数据源接入（可和LangGraph并行学习） |

---

## 2. LangChain 在解决什么
### 2.1 一句话定位
LangChain = **模型调用 + 提示模板 + 输出解析器 + 工具/检索插件** 的上层组合框架。开发者聚焦**数据流转步骤**，不需要重复手写HTTP请求、拼接Prompt的底层逻辑。

### 2.2 核心积木组件
| 积木 | 作用（直觉理解） |
|------|----------------|
| **Model / ChatModel** | 对接各类大模型，兼容OpenAI、本地开源模型等 |
| **PromptTemplate** | 变量渲染，产出可复用提示词 |
| **OutputParser** | 将模型自由文本解析为JSON等结构化对象 |
| **Chain / Runnable** | 将多个可运行单元组装为数据管道 |
| **Memory** | 管理多轮对话历史，控制写入上下文的内容 |
| **Tools** | 可供模型调用的外部函数：搜索、数据库、第三方API |
| **Retrievers / VectorStore** | 检索文档片段，支撑RAG问答 |

现代LangChain推荐使用 **LCEL（LangChain Expression Language）**，通过管道符 `|` 串联Runnable组件，原生支持流式输出、批量调用、异步执行、链路追踪。
...
prompt = ChatPromptTemplate.from_messages([("user", "回答问题：{question}")])
model = ChatOpenAI(model="gpt-4o-mini")
parser = JsonOutputParser()

# LCEL管道组装
chain = prompt | model | parser
result = chain.invoke({"question": "解释什么是Embedding"})
```
...
对应 §2.2 的 LCEL 写法：`prompt | model | parser`，一次 `invoke` 跑完整条管道。
