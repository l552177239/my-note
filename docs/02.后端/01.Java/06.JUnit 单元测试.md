...
---
## 13. 下一篇
下一篇：**Lombok 与日志框架 SLF4J + Logback**。  
之后按路线：**Spring（IoC / DI / AOP）→ SpringBoot 快速入门**。
重点预习：
- 构造器注入会大量用到 `@RequiredArgsConstructor`
- 日志门面 SLF4J + 实现 Logback
- 不要用 `System.out.println` 调试 / 打日志
